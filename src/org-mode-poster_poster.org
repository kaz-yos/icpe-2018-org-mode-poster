...
*** Notations and Definitions                                       :B_block:
\small
...
Please note we defined a PS for each treatment option (generalized PS cite:imbens_role_2000).
...
- We used the following as working definitions.
...
- These triangles are ternary scatter plots (cite:HamiltonggternExtensionggplot22017) of individuals in three groups. Being close to a corner means a high propensity of being in that group.
...
:END:
\small
#+ATTR_LATEX: :height 0.6\textheight :width 0.6\textwidth :options page=1,keepaspectratio :center t
